...
class CssParser
{
     ...
     */
    private function consumeDeclaration(): ?array
    {
        if ($this->t->peek() === null) {
            return null;
        }

        // Declaration構造体を初期化する（今回は配列で代用）
        $declaration = [
            'property' => '',
            'value' => null,
        ];
        ...
        $declaration['property'] = $this->consumeIdent();
        ...
        $declaration['value'] = $this->consumeComponentValue();
     ...
     * Consume a selector
     */
    private function consumeSelector(): string
    {
        $token = $this->t->next();
        if ($token === null) {
            throw new \RuntimeException("should have a token but got None");
        ...
        switch ($token->type) {
            case CssTokenType::HashToken:
                ...
                return substr($value, 1); // Remove the '#' prefix
     ...
     */
    private function consumeQualifiedRule(): ?array
    {
        $rule = [
            'selector' => '',
            'declarations' => [],
        ...
        while (true) {
            ...
            if ($token->type === CssTokenType::OpenCurly) {
                ...
                $rule['declarations'] = $this->consumeListOfDeclarations();
                return $rule;
            } else {
                $rule['selector'] = $this->consumeSelector();
            }
        }
    }
